...
func main() {
    ...
    for _, device := range cfg.Devices {
        ...
        if nb != nil && !cfg.NetBox.DryRun {
            ...
            // Detect and import device type from SNMP sysDescr
            var deviceTypeID float64
            deviceType, err := nb.DetectAndImportDeviceType(deviceInfo.SysDescr)
            if err != nil {
                log.Printf("⚠ Could not detect/import device type: %v", err)
            } else if deviceType != nil {
                log.Printf("✓ Device type: %s %s (ID: %d)", deviceType.Manufacturer.Name, deviceType.Model, deviceType.ID)
                deviceTypeID = float64(deviceType.ID)
            }
            ...
            deviceID, err := nb.PushDeviceWithType(deviceInfo, cfg.NetBox.Site, deviceRole, deviceTypeID)
            if err != nil {
